...
See [Agent Skills Standard Mapping](agent-skills-standard-mapping.md) for the
generic Capability module → open package → vendor/runtime implementation chain.
...
| Capability modules | [`templates/SKILL.md`](../templates/SKILL.md) | A capability module can use an Agent Skills-compatible package; OpenAI Skills are a compatible vendor/runtime implementation with product-specific upload, version, execution, limit, and security behavior. | Claude Code skills follow the open standard and add implementation-specific invocation, permission, subagent, dynamic-context, and frontmatter behavior. | No direct skill primitive. A skill may use MCP tools and resources, but it is not itself an MCP tool. | Can be represented differently as reusable subgraphs, node procedures, runnable packages, or documented task modules. LangSmith can evaluate behavior that uses one. | Preserve Capability module → Agent Skills-compatible package → vendor/runtime implementation. A skill explains reusable procedure; a tool performs a callable action. See the [dedicated mapping](agent-skills-standard-mapping.md). |
